...
"""
class TaskGroupGetDaySurvivalFood(TaskGroupDaily):
    ...
    def __init__(self, group, need_type, need_level):
        ...
        self.amount_food_to_collect = 0

        # total amount of cell to collect for whole day
        self.amount_food_collect_today = 0

        # total amount of cell to collected so far this day
        self.amount_food_collected_today = 0


        self.food_storage = self.contents[ResourceType.FOOD]
    ...
    def trigger(self, group):
        ...
        if self.last_day_triggered != TimeStamp().day:
            ...
            # Max amount of food we need today
            self.food_storage.max_amount = global_params.hunger_tick * \
                                            global_params.daily_ticks * \
                                            len(self.group.members) * \
                                            global_params.supply_days


            # net amount of food to collect given start of dat storage
            self.amount_food_to_collect = max(0,  self.food_storage.max_amount - self.food_storage.storage)

            # stats
            self.amount_food_collect_today = self.amount_food_to_collect
        ...
        self.amount_food_collected_today += self.food_storage.amount_collected

        # set amount to collect to account for previous collection
        self.amount_food_to_collect = max(0, self.amount_food_to_collect - self.food_storage.amount_collected)

        # reset storage stats
        self.reset_storage_statistics()

        # set goal for current session
        self.food_storage.amount_goal = self.amount_food_to_collect
    ...
    def possible_actions(self):
        ...
        actions = []
        if self.amount_food_to_collect > 0:
            actions.append(ActionGroupHourlySkilledGetResource(ResourceType.FOOD, self.amount_food_to_collect, self))

        return [actions]

    def __repr__(self):
        start = str("Survival food supply %s days (f: %s/%s)" % (global_params.supply_days,
                                                                      int(self.food_storage.storage),
                                                                      int(self.food_storage.max_amount)))
        if self.amount_food_collect_today == 0:
            return start
        return start + " | \n    food %s/%s (%s%%)" % (int(self.amount_food_collected_today + self.food_storage.amount_collected),
                                                                          int(self.amount_food_collect_today),
                                                                          int(((self.amount_food_collected_today + self.food_storage.amount_collected)
                                                         / self.amount_food_collect_today) * 100))