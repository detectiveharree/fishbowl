...
class ArmyTerritoryAnnexDefence(ArmyDefence):
    ...
    def get_campaign_response(group, enemy_territory_campaign):
        ...
        if fight:
            defensive_strategy = ArmyTerritoryAnnexDefence(group, enemy_territory_campaign)
    ...
    def get_army_rally_point(self, original_group):
        # halfway point between group and territory location enemy gonna take over
        route = list(pathfinding.get_route(self.original_group.stockpile_location, self.enemy_territory_campaign.territory_location))
        return route[max(round(len(route) / 2), len(route) - 1)]
    ...
    def battle_over(self, result):
        if result == BATTLE_RESULT.WON:
            logging.info("%s won the battle against %s keeping its territory" % (self.original_group.id_number, self.enemy_territory_campaign.original_group.id_number))
        else:
            logging.info("%s lost the battle against %s losing some territory" % (self.original_group.id_number, self.enemy_territory_campaign.original_group.id_number))
